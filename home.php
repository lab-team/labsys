...
include dirname(__FILE__).'/include/UserDb.php';
...
if (! isset($_SESSION['name'])) {
    ...
    $name = $_SESSION['name'];
    $user = new UserDb();
    $stu_num = $user->selectUserByUser_name($name)['student_num'];
	 //echo "<script>alert($stu_num);</script>";
	 $conn = new mysqli($ghostname, $gpassname, $gpassword, $gdatabase);
	 if($conn->connect_error){
	           die("Connect Failed, the error is:".$conn->connect_error);
	 }
	 $setcharset ="set names 'utf8'";
	 mysqli_query($conn, $setcharset);
	
    $query_name = "select * from stureg where student_num='$stu_num'";
    ...
    //echo "<script>alert('denglu');location='testqiaodao.php?date=$regdate[0]';</script>";
    //echo "<script>alert($regdate[0]);</script>";
    //echo "<script>alert($regdate[1]);</script>";
}
}
...
<html>
	...
	function changeit(url){
	 ...
	 function makedatepicker(){
        ...
        drp = $("#myDate").datepicker({daterange:daterange, disableddates:disableddates, defaultDate: new Date(date_str)});
	 }
